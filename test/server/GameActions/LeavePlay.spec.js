const LeavePlay = require('../../../server/game/GameActions/LeavePlay');

describe('LeavePlay', function() {
    beforeEach(function() {
        this.cardSpy = jasmine.createSpyObj('card', ['allowGameAction', 'leavesPlay']);
        this.cardSpy.location = 'play area';
        this.nonTerminalAttachmentSpy = jasmine.createSpyObj('nonTerminalAttachment', ['isTerminal']);
    ...
    describe('createEvent()', function() {
        ...
        describe('the event handler', function() {
            beforeEach(function() {
                ...
                this.attachedEvents = this.event.attachedEvents.map(event => ({ name: event.name, card: event.card }));
            });
            ...
            it('attaches discard events for the dupes', function() {
                expect(this.attachedEvents).toContain({ name: 'onCardDiscarded', card: this.dupeSpy });
            });

            it('attaches discard events for terminal attachments', function() {
                expect(this.attachedEvents).toContain({ name: 'onCardDiscarded', card: this.terminalAttachmentSpy });
            });

            it('attaches return to hand events for non-terminal attachments', function() {
                expect(this.attachedEvents).toContain({ name: 'onCardReturnedToHand', card: this.nonTerminalAttachmentSpy });
            });
        });
    });
});
